...
dfqsubs<-read.csv("../substitution_AA/ram_ecoli/combined_str0_8/txt/detct_quant.csv")

dfqsubs_flt<-mclapply(1:nrow(dfqsubs), function(i){
  dfBP<-dfqsubs[i,]%>%dplyr::select(c(starts_with("Base.intensity")))%>%t() %>%
    as.data.frame()%>%rownames_to_column()%>%
    separate(col = "rowname",into = c("a","sample"),sep=".Exp..")%>%dplyr::select(2,3)
  colnames(dfBP)[2]<-"bp"
  ...
  dfmerg<-merge(dfBP,dfDP)%>%rowwise%>%mutate(dfratio=dp/sum(dp,bp,na.rm = T))%>%
    filter(dfratio<0.01)
  
  young<-dfmerg%>%filter(grepl("wt",sample,ignore.case=T))%>%getElement("dfratio")%>%mean(na.rm=T)
  old<-dfmerg%>%filter(grepl("ep",sample,ignore.case=T))%>%getElement("dfratio")%>%mean(na.rm=T)
  
  return(data.frame(young=young,old=old))
  
},mc.cores = 20)%>%rbind.fill()%>%filter(young >0 | old >0)%>%na.omit()
...
##########
range(dfqsubs_flt)
misincorp <- lapply(10 ** c(-4,-3.6,-3.3,-3),function(thres){
  dfdat<-dfqsubs_flt %>%filter(young > thres | old > thres) %>% mutate(thres = thres) %>%
    mutate(mw.p = wilcox.test(young, old, paired=T,alternative = "l")$p.value)
}) %>% rbind.fill()
misincorp$mw.p%>%unique()
dfplot<-misincorp%>%reshape2::melt(id.vars =c("thres","mw.p") )

Fig1c<-ggplot(dfplot,aes(x=factor(thres),y=value,fill=variable))+
  geom_boxplot(outlier.colour=NA,position=position_dodge(0.8),
               key_glyph="rect",width=0.5,lwd=0.2,fatten =0.8)+
  stat_boxplot(geom = "errorbar",aes(ymin=..ymax..), width=0.2,lwd=0.2,outlier.shape = NA,
               position =position_dodge(0.8))+
  ...
  theme_test()+
  theme(legend.position = "top",legend.key   = element_blank(),legend.key.size = unit(3,"mm"))+
  scale_y_continuous(breaks=c(0.0000,0.0025,0.0050), labels = c("0.0000","0.0025","0.0050"),
                     limits =c(0.0000,0.0050) )+
  scale_x_discrete(labels = c(expression(10**-4),expression(10**-3.6),expression(10**-3.3),expression(10**-3)),
                   breaks = c(10**-4,10**-3.6,10**-3.3,10**-3))+
  annotate("text",x=1,y=0.004,label=expression(italic(P) * " = " * "2.4×" * 10**-10),size=2)+
  annotate("text",x=2,y=0.004,label=expression("4.0×" * 10**-9),size=2)+
  annotate("text",x=3,y=0.004,label=expression("8.1×" * 10**-8),size=2)+
  annotate("text",x=4,y=0.004,label=expression("6.7×"*10**-6),size=2)
  
  


# Fig1.e -----------------------------------------------------------------
load("../readthrough/RData.file/rt_common.RData")
load("../readthrough/RData.file/readcount_yeastPositive.RData")
cds_m5 = 15;cds_m3 = 33;exten_m5 = 0
compRep <- list() # Composite replicates read count
names(readcount)
compRep$Mut<- composite(readcount[ names(readcount)[1:2]])
...
WT<-RTscore$WT %>%select(transcript,WT=rte_ext)
Mut<-RTscore$Mut %>% select(transcript,Mut=rte_ext)



dfmrg<-merge(WT,Mut)%>%dplyr::select(2,3)%>%filter(WT<0.1 & Mut<0.1 )

median(dfmrg$WT)
median(dfmrg$Mut)
wilcox.test(dfmrg$WT,dfmrg$Mut,paired = T, alternative = "l")$p.value
#p-value = 3.079473e-39
range(dfmrg)

readthr <- lapply(10 ** c(-3,-2.5,-2,-1.5),function(thres){
  dfdat<-dfmrg %>%filter(WT > thres | Mut > thres) %>% mutate(thres = thres) %>%
    mutate(mw.p = wilcox.test(WT, Mut, paired=T,alternative = "l")$p.value)
}) %>% rbind.fill()

readthr$mw.p%>%unique()
dfplot<-readthr%>%reshape2::melt(id.vars =c("thres","mw.p") )

Fig1e<-ggplot(dfplot,aes(x=factor(thres),y=value,fill=variable))+
  geom_boxplot(outlier.colour=NA,position=position_dodge(0.8),
               key_glyph="rect",width=0.5,lwd=0.2,fatten =0.8)+
  stat_boxplot(geom = "errorbar",aes(ymin=..ymax..), width=0.2,lwd=0.2,outlier.shape = NA,
               position =position_dodge(0.8))+
  ...
  theme_test()+
  theme(legend.position = "top",legend.key   = element_blank(),legend.key.size = unit(3,"mm"))+
  scale_y_continuous(breaks=c(0.000,0.050,0.100), labels = c(0.000,0.050,0.100),
                     limits =c(0.0000,0.12) )+
  scale_x_discrete(labels = c(expression(10**-3),expression(10**-2.5),expression(10**-2),expression(10**-1.5)), 
                   breaks = c(10**-3,10**-2.5,10**-2,10**-1.5))+
  annotate("text",x=1.1,y=0.11,label=expression(italic(P) * " = " * "7.9×" * 10**-27),size=2)+
  annotate("text",x=2.1,y=0.11,label=expression("1.1×" * 10**-26),size=2)+
  annotate("text",x=3,y=0.11,label=expression("2.7×" * 10**-25),size=2)+
  annotate("text",x=4,y=0.11,label=expression("2.1×" * 10**-18),size=2)

  

# 合并图 ---------------------------------------------------------------------
library(cowplot)
Fig1<-plot_grid("","",Fig1c,"","",Fig1e,ncol = 3)  
ggsave("Fig1.pdf",Fig1,width =8, height =5)
